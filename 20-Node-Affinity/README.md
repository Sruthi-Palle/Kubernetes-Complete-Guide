...
1. **Required During Scheduling, Ignored During Execution**
   - The pod is scheduled only on nodes that fully satisfy the affinity rules. (If all existing nodes doesn't match affinity rules, the pod will not be scheduled)
