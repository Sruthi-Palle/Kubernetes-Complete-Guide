...
> 💡 This article explores how taints and tolerations control pod scheduling on specific nodes in Kubernetes. In Kubernetes, a taint is applied to a node to repel pods unless they have a matching toleration . This ensures that only intended pods can run on tainted nodes.
...
![alt text](../Images/Taints-and-Tolerations.png)
...
- **NoExecute:** New pods without a matching toleration will not be scheduled, and existing pods without a matching toleration will be evicted from the node.
...
![alt text](../Images/Taints-and-Tolerations-1.png)

> 💡 Taints and tolerations are used to ensure that pods are not accidentally scheduled on nodes that should run only specific workloads, but they do not guarantee that a pod will run on a particular node. For Example, we cannot guarantee that the pod with matching toleration for a tainted node, can be placed only on that tainted node, it might be scheduled on other nodes as well. For node-specific scheduling, consider using node affinity.
...
Now that you understand taints and tolerations, experiment with their configuration in your Kubernetes cluster to better control pod placement.
...
## You can refer practical examples of using Taints and Tolerations in kubernetes

- [Demo-Taints-and-Tolerations](../18-Taints-and-Tolerations/Demo-Taints-and-Tolerations.md)
